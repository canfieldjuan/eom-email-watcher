...
class GatewayModel:
            ...
            with (
                ...
                if expected_request_id is not None:
                    try:
                        ...
                        raise
                    if response.status_code >= 400 or response_payload.get("status") == "failed":
    ...
    @staticmethod
    def _gateway_error(payload: dict[str, object], expected_request_id: str) -> GatewayModelError:
    ...
    def health(self) -> tuple[bool, str]:
        try:
            payload = self._request("GET", "/v1/health", timeout=GATEWAY_HEALTH_TIMEOUT_SECONDS)
